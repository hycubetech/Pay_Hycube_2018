...
use Hycpay\Client\Adapter\AdapterInterface;
use Hycpay\Network\NetworkInterface;
use Hycpay\TokenInterface;
use Hycpay\InvoiceInterface;
use Hycpay\PayoutInterface;
use Hycpay\Util\Util;
use Hycpay\PublicKey;
use Hycpay\PrivateKey;

/**
 * Client used to send requests and receive responses for HycPay's Web API
 *
 * @package Hycpay
 */
class Client implements ClientInterface
{
    ...
    /**
     * Set the Public Key to use to help identify who you are to HycPay. Please
    ...
    {
        # HycPay returns the invoice time in milliseconds. PHP's DateTime object expects the time to be in seconds
        ...
        $invoice
            ...
            ->setPrice($data['price'])
            ->setCurrency(new \Hycpay\Currency($data['currency']))
    ...
    public function getPayouts($status = null)
    {
        ...
        array_walk($body['data'], function ($value, $key) use (&$payouts) {
            $payout = new \Hycpay\Payout();
            $payout
                ->setId($value['id'])
                ->setAccountId($value['account'])
                ->setCurrency(new \Hycpay\Currency($value['currency']))
            ...
            array_walk($value['instructions'], function ($value, $key) use (&$payout) {
                $instruction = new \Hycpay\PayoutInstruction();
                $instruction
                    ->setId($value['id'])
                ...
                array_walk($value['transactions'], function ($value, $key) use (&$instruction) {
                    $transaction = new \Hycpay\PayoutTransaction();
                    $transaction
    ...
    public function createToken(array $payload = array())
    {
        ...
        if (isset($body['error'])) {
            throw new \Hycpay\Client\HycpayException($this->response->getStatusCode().": ".$body['error']);
        }
    ...
    /**
     * Returns the Response object that HycPay returned from the request that
     * was sent
     *
    ...
    /**
     * Returns the request object that was sent to HycPay
     *
    ...
    public function sendRequest(RequestInterface $request)
    {
        if (null === $this->adapter) {
            ...
            $this->adapter = new \Hycpay\Client\Adapter\CurlAdapter();
        }
    ...
    protected function prepareRequestHeaders(RequestInterface $request)
    {
        ...
        $request->setHeader('X-HycPay-Plugin-Info', sprintf('%s/%s', self::NAME, self::VERSION));
