...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder()
    {
        ...
        $rootNode    = $treeBuilder->root('hycpay');
        $rootNode
            ->children()
                ->scalarNode('public_key')
                    ->info('Public Key Filename')
                    ->defaultValue(getenv('HOME').'/.hycpay/api.pub')
                ->end()
                ->scalarNode('private_key')
                    ->info('Private Key Filename')
                    ->defaultValue(getenv('HOME').'/.hycpay/api.key')
                ->end()
                ->scalarNode('sin_key')
                    ->info('Private Key Filename')
                    ->defaultValue(getenv('HOME').'/.hycpay/api.sin')
                ->end()
                ->enumNode('network')
     ...
     *
     * key_storage MUST:
     *     * implement Hycpay\Storage\StorageInterface
    ...
    protected function addKeyStorageNode()
    {
        ...
        $node
            ...
            ->defaultValue('Hycpay\Storage\EncryptedFilesystemStorage')
            ->validate()
                ->always()
                ->then(function ($value) {
                    ...
                    // requires PHP >= 5.3.7
                    if (!is_subclass_of($value, 'Hycpay\Storage\StorageInterface')) {
                        throw new \Exception(
                            sprintf(
                                '"%s" does not implement "Hycpay\Storage\StorageInterface"',
                                $value
                            )
                        );
                    }
